...
if exists('g:autoloaded_calendar')
  finish
endif
let g:autoloaded_calendar = 1
...
if !exists("g:calendar_diary")
  let g:calendar_diary = "~/.local/share/vim_diary"
endif
...
" Default Calendar Key Bindings: {{{1
...
" CalendarDoAction: call the action handler function {{{1
...
function! calendar#show(...)
  ...
  if vwinnum >= 0
    ...
    " or not
    if dir
      silent execute 'to '.vheight.'split __Calendar'
...
" Diary: calendar hook function {{{1
"********************************
...
" CalendarBuildKeymap: build keymap {{{1
...
" CalendarHelp: show help for Calendar {{{1
