...
bindAlias newCombinations binding = do
 ...
 where
  newBinding c = Binding
    { combination = c
    , explanation = AliasExplanation (combination binding)
...
storeBindings explainableBindings xConfig = xConfig
  ...
  store = do
    ...
    if oldDoc == initialValue then XMES.put newDoc else XMES.modify (<> newDoc)
...
mapBindings binder =
  let bindMap xConfig = runBinder $ binder xConfig
      bindings =
        Map.fromList
