...
module XMonad.Local.Bindings.Bind
    ( Binder
    , (|/-)
    , (^>)
    , (...)
...
buttonMaskToString m = maybe "" (<> "-") $ Map.lookup m modMap
  where modMap = Map.fromList [ (mod1Mask,                               "Alt")
...
explain expl = case expl of
  ...
  where paddingAmount = maxKeysStringLen + length "(alias: )"
        padding       = repeat ' '

describe :: Binding -> String
describe binding = keyString <> explain (explanation binding)
  where keyString = take maxKeysStringLen $ keyCombinationToString (combination binding) <> repeat ' '
...
instance ExtensionClass DocBindings
  where initialValue = DocBindings "no Bindings stored\n"
...
getBindings = do DocBindings doc <- XMES.get
                 return doc

mapBindings :: (XConfig Layout -> Binder a) -> (XConfig Layout -> KeyMap, X DocBindings)
mapBindings binder = let bindMap xConfig = runBinder $ binder xConfig
                         bindings = Map.fromList . fmap ((modifier &&& key) . combination &&& action) . bindMap
