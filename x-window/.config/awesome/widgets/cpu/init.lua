...
-- theme_path.."/wallpapers/zenburn.png"
local CMD = [[sh -c "grep '^cpu.' /proc/stat; ps -eo '%p|%c|%C|' -o "%mem" -o '|%a' --sort=-%cpu ]]
    .. [[| head -11 | tail -n +2"]]

local WIDGET_DIR = gfs.get_configuration_dir() .. '/widgets/cpu/'
...
local function worker(user_args)
  ...
  local step_spacing            = args.step_spacing or 0
  local color                   = args.color or beautiful.fg_focus
