...
" }}}
" Mappings: {{{2
nnoremap <silent> <Plug>ShowMarksToggle    :ShowMarksToggle<CR>
nnoremap <silent> <Plug>ShowMarksOn        :ShowMarksOn<CR>
nnoremap <silent> <Plug>ShowMarksClearMark :ShowMarksClearMark<CR>
nnoremap <silent> <Plug>ShowMarksClearAll  :ShowMarksClearAll<CR>
nnoremap <silent> <Plug>ShowMarksPlaceMark :ShowMarksPlaceMark<CR>

if !hasmapto('<Plug>ShowMarksToggle')
  map <unique> <S-F10> <Plug>ShowMarksToggle
  map <unique> <F22>   <Plug>ShowMarksToggle
endif
if !hasmapto('<Plug>ShowMarksOn')
  map <unique> <Leader>mo <Plug>ShowMarksOn
endif
if !hasmapto('<Plug>ShowMarksClearMark')
  map <unique> <Leader>mc <Plug>ShowMarksClearMark
endif
if !hasmapto('<Plug>ShowMarksClearAll')
  map <unique> <Leader>ma <Plug>ShowMarksClearAll
endif
if !hasmapto('<Plug>ShowMarksPlaceMark')
  map <unique> <Leader>mm <Plug>ShowMarksPlaceMark
endif

noremap <unique> <script> <Leader>sm m
noremap <silent> m :exe 'norm <Leader>sm'.nr2char(getchar()) <Bar> call <SID>ShowMarks()<CR>
" }}}
" }}}
...
hi default link SignColumn ShowMarksHLl
hi default link SignColumn ShowMarksHLu
hi default link SignColumn ShowMarksHLo
hi default link SignColumn ShowMarksHLm
" }}}
...
" IncludeMarks: {{{2
" Description: {{{3
"   This function returns the list of marks (in priority order) to
...
" NameOfMark: {{{2
" Description: {{{3
"   Convert marks that cannot be used as part of a variable name to
...
" Parameters:
"   mark - Specifies the mark to find the name of.
" Returns:
"   The name of the requested mark.
" }}}

function! s:NameOfMark(mark)
  let name = a:mark
  if a:mark =~# '\W'
...
" VerifyText: {{{2
" Description: {{{3
"   Verify the validity of a showmarks_text{upper,lower,other} setup variable.
"   Default to ">" if it is found to be invalid.
" Parameters:
"   which - Specifies the variable to verify.
" }}}

function! s:VerifyText(which)
...
" ShowMarksSetup: {{{2
" Description: {{{3
"   This function sets up the sign definitions for each mark.
...
" ShowMarksOn: {{{2
" Description: {{{3
"   Enable showmarks, and show them now.
" }}}

function! s:ShowMarksOn()
...
" ShowMarksToggle: {{{2
" Description: {{{3
"   This function toggles whether marks are displayed or not.
" }}}
...
" ShowMarksClearMark: {{{2
" Description: {{{3
"   This function hides the mark at the current line.
"   It simply moves the mark to line 1 and removes the sign.
"   Only marks a-z and A-Z are supported.
" }}}

function! s:ShowMarksClearMark()
  let ln = line(".")
  let n = 0
  let s:maxmarks = strlen(s:IncludeMarks())
  while n < s:maxmarks
...
" ShowMarksClearAll: {{{2
" Description: {{{3
"   This function clears all marks in the buffer.
"   It simply moves the marks to line 1 and removes the signs.
"   Only marks a-z and A-Z are supported.
" }}}

function! s:ShowMarksClearAll()
  let n = 0
  let s:maxmarks = strlen(s:IncludeMarks())
  while n < s:maxmarks
    let c = strpart(s:IncludeMarks(), n, 1)
...
" ShowMarksHideAll: {{{2
" Description: {{{3
"   This function hides all marks in the buffer.
...
" ShowMarksPlaceMark: {{{2
" Description: {{{3
"   This function will place the next unplaced mark (in priority
