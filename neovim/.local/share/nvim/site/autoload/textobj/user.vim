...
function! textobj#user#plugin(plugin_name, obj_specs)  abort "{{{1
  ...
  execute
        \ 'command! -bang -bar -nargs=0 Textobj'.Plugin.'DefKeyMaps'
  ...
  \  && (!exists('g:textobj_'.plugin.'_no_default_key_mappings'))
    execute 'Textobj'.Plugin.'DefKeyMaps'
  endif
    ...
    for [spec_name, spec_info] in items(specs)
      ...
      elseif spec_name =~# '^select\(\|-[ai]\)$'
        ...
        throw printf('Unknown property: %s given to %s',
              \ string(spec_name), string(obj_name))
      endif
      unlet spec_info  " to avoid E706.
    endfor
  endfor

  call s:define_failsafe_key_mappings(self.name, self.obj_specs)
endfunction
" }}}
...
function! s:plugin.do_by_pattern(spec_name, obj_name, previous_mode) abort " {{{1
  ...
  call {s:PATTERN_IMPL_TABLE[a:spec_name]}(
        \ specs['pattern'], flags, a:previous_mode)
endfunction
" }}}
      ...
      if spec_name =~# '-function$'
        ...
        if spec_info =~# '^s:'
          if has_key(specs, 'sfile')
            let specs[spec_name] = substitute(
                  \ spec_info, '^s:', s:snr_prefix(specs['sfile']), '')
          else
